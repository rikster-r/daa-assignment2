...
/**
 * Benchmark tests for InsertionSort that export results to CSV files.
 * These tests generate performance data for analysis.
 */
class InsertionSortBenchmarkTest {

    private static final String CSV_FILE = "insertion_sort_benchmark.csv";
    ...
    void benchmarkComprehensive() {
        ...
        System.out.println("\n=== Comprehensive Insertion Sort Benchmark ===");
    ...
    private void runBenchmarkForSize(int size) {
        ...
        for (String inputType : inputTypes) {
            ...
            // Export to CSV
            tracker.exportToCSV(CSV_FILE, "InsertionSort", size);
            ...
            // Verify correctness
            assert InsertionSort.isSorted(arr) : "Array should be sorted after insertion sort";
        }
    }
    ...
    void benchmarkBestVsWorst() {
        ...
        InsertionSort.sort(sortedArr, bestCaseTracker);
        bestCaseTracker.stop();
        bestCaseTracker.exportToCSV(CSV_FILE, "InsertionSort-BestCase", size);
        ...
        InsertionSort.sort(reversedArr, worstCaseTracker);
        worstCaseTracker.stop();
        worstCaseTracker.exportToCSV(CSV_FILE, "InsertionSort-WorstCase", size);
    ...
    @Test
    @DisplayName("Benchmark: Nearly sorted optimization test")
    void benchmarkNearlySortedOptimization() {
        int size = 1000;

        System.out.println("\n=== Nearly Sorted Optimization Test ===");
        ...
        InsertionSort.sort(nearlySortedArr, nearlySortedTracker);
        nearlySortedTracker.stop();
        nearlySortedTracker.exportToCSV(CSV_FILE, "InsertionSort-NearlySorted", size);

        // Random
        int[] randomArr = generateArray(size, "random");
        PerformanceTracker randomTracker = new PerformanceTracker();
        randomTracker.start();
        InsertionSort.sort(randomArr, randomTracker);
        randomTracker.stop();
        randomTracker.exportToCSV(CSV_FILE, "InsertionSort-Random", size);
        ...
        System.out.printf("%nNearly sorted should be significantly faster than random%n");
        System.out.printf("Comparison reduction: %.1f%%%n",
                100.0 * (1 - (double) nearlySortedTracker.getComparisons() / randomTracker.getComparisons()));
    }
}